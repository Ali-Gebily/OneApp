...
{
    /// <summary>
    ///     Mock based user store implementation that supports IUserStore, IUserLoginStore, IUserClaimStore and
    ///     IUserRoleStore
    /// </summary>
    /// <typeparam name="TUser"></typeparam>
    internal class MockUserStore<TUser> :
        MockUserStore<TUser, MockIdentityRole, string, MockIdentityUserLogin, MockIdentityUserRole, MockIdentityUserClaim>,
        IUserStore<TUser> where TUser : MockIdentityUser
    {
        /// <summary>
    ...
    internal class MockUserStore<TUser, TRole, TKey, TUserLogin, TUserRole, TUserClaim> :
        ...
        where TUser : MockIdentityUser<TKey, TUserLogin, TUserRole, TUserClaim>
        where TRole : MockIdentityRole<TKey, TUserRole>
        where TUserLogin : MockIdentityUserLogin<TKey>, new()
        where TUserRole : MockIdentityUserRole<TKey>, new()
        where TUserClaim : MockIdentityUserClaim<TKey>, new()
    {
        private readonly IList<TUserLogin> _logins;
        private readonly MockEntityStore<TRole,TKey> _roleStore;
        private readonly IList<TUserClaim> _userClaims;
        private readonly IList<TUserRole> _userRoles;
         private MockEntityStore<TUser,TKey> _userStore;
        ...
        public MockUserStore()
        {
            ...
            _userStore = new MockEntityStore<TUser,TKey>( );
            _roleStore = new MockEntityStore<TRole,TKey>( );
            _logins = new List<TUserLogin>();
            _userClaims = new List<TUserClaim>();
            _userRoles = new List<TUserRole>();
        }
        public void Dispose()
        {
        ...
        public virtual async Task<TUser> FindAsync(UserLoginInfo login)
        {
            ...
            var userLogin =
                  _logins.FirstOrDefault(l => l.LoginProvider == provider && l.ProviderKey == key);
            if (userLogin != null)
            {
        ...
        public virtual async Task<IList<string>> GetRolesAsync(TUser user)
        {
            ...
            var query = from userRole in _userRoles
                        ...
                        select role.Name;
            return query.ToList() as IList<string>;
        }
        ...
        public virtual async Task<bool> IsInRoleAsync(TUser user, string roleName)
        {
            ...
            var role = _roleStore.DbEntitySet.FirstOrDefault(r => r.Name.ToUpper() == roleName.ToUpper());
            if (role != null)
            {
                var userId = user.Id;
                var roleId = role.Id;
        ...
        protected virtual async Task<TUser> GetUserAggregateAsync(Expression<Func<TUser, bool>> filter)
        {
            TKey id;
            TUser user;
            if (MockFindByIdFilterParser.TryMatchAndGetId(filter, out id))
            {
        ...
        private static class MockFindByIdFilterParser
        {
